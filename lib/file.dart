...
class Post with ChangeNotifier {
  ...
  Post({
    ...
    required this.postCreator, // Add the postCreator property
...
class _PostListScreenState extends State<PostListScreen> {
  ...
  Future<void> _updateEmoji(Post post, String emoji) async {
    if (post.reactions[emoji] != null) {
      if (emojiClicked[emoji] ?? false) {
        post.reactions[emoji]!.value -=
            1; // Reduce the count if the emoji was clicked
      } else {
        post.reactions[emoji]!.value +=
            1; // Increase the count if the emoji was not clicked
      }
    }
    ...
    if (currentUserId != null &&
      ...
      try {
        ...
        if (response.statusCode == 200) {
          ...
          // Refresh posts to get updated comments for all posts after deleting the comment
          await refreshPosts();
        } else {
  ...
  Future<void> fetchPosts() async {
    ...
    try {
      ...
      if (userId != null) {
        final response = await http.get(
          Uri.parse(
              '$baseUrl/posts/user/$userId'), // Use the correct route with the userId
          headers: {'Accept': 'application/json'},
        );
        // Check if the flag is still true before proceeding
...
// Modified createComment function with comment and post refresh
  Future<void> createComment(
    ...
    try {
      ...
      if (response.statusCode == 201) {
        ...
        // Refresh comments after a short delay (adjust the duration as needed)
        await Future.delayed(Duration(milliseconds: 1000)); // Wait for 1 second
        // Now, refresh the posts to get updated comments for all posts
        await refreshPosts();
      } else {
                  ...
                  Row(
                    ...
                    children: [
                      IconButton(
                        ...
                        onPressed: () {
                          _showEditPostDialog(post.id, post.content);
                        },
                      ),
                      IconButton(
